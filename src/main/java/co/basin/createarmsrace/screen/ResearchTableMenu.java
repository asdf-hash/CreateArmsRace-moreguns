...
public class ResearchTableMenu extends AbstractContainerMenu {
    ...
    public ResearchTableMenu(int id, Inventory inv, BlockEntity entity, ContainerData data) {
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(handler -> {
            this.addSlot(new SlotItemHandler(handler, 0, 40, 36));
            this.addSlot(new SlotItemHandler(handler, 1, 122, 36));
        });
    ...
    public double getTimeRemaining() {
        int progress = this.data.get(0);
        int maxProgress = this.data.get(1);
        return (maxProgress - progress) / 72000d;
    }

    public int getResearchSelectionIndex() {
